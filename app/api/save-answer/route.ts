...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId || !questionId || !userAnswer) {
      return NextResponse.json({ error: "Missing required fields: userId, questionId, userAnswer" }, { status: 400 })
    }
    ...
    try {
      const updateCommand = new UpdateCommand({
        ...
        UpdateExpression: "SET answers.#qid = :answerData, updated_at = :updatedAt",

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId || !questionId || llmRating === undefined) {
      return NextResponse.json({ error: "Missing required fields: userId, questionId, llmRating" }, { status: 400 })
    }
    ...
    try {
      const updateCommand = new UpdateCommand({
        ...
        UpdateExpression: "SET ratings.#qid = :rating, questions.#qid.#status = :status, updated_at = :updatedAt",
